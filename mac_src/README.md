...
The macOS build of DotfileDesk: a Tauri 2 + React/TypeScript app for discovering, snapshotting,
...
| Xcode Command Line Tools | macOS build toolchain |
...
`~/Library/Application Support/dev.hannesgnann.dotfiledesk/` — a SQLite database
...
| `npm run tauri dev` | runs the real macOS app |
...
mac_src/
├── docs/
│   └── ARCHITECTURE.md   macOS-specific architecture (command layer, plugins, packaging)
...
[`../crates/dotfiledesk-core`](../crates/dotfiledesk-core) and is shared with `linux_src` and
...
| `README.md` | macOS usage, requirements, and quickstart (this file) |
| [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) | macOS-specific technical architecture |
